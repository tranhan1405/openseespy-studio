...
class FrameWizard(QWizard):
    ...
    def _build_geometry_page(self) -> None:
        ...
        self.preset_duplicate = QPushButton("Duplicate…")
        self.preset_rename = QPushButton("Rename…")
        self.preset_delete = QPushButton("Delete")
        self.preset_import = QPushButton("Import JSON…")
        self.preset_export = QPushButton("Export JSON…")

        preset_primary = QWidget()
        preset_primary_layout = QHBoxLayout(preset_primary)
        preset_primary_layout.setContentsMargins(0, 0, 0, 0)
        preset_primary_layout.addWidget(self.preset_load)
        preset_primary_layout.addWidget(self.preset_save)
        preset_primary_layout.addWidget(self.preset_duplicate)
        preset_primary_layout.addStretch(1)

        preset_manage = QWidget()
        preset_manage_layout = QHBoxLayout(preset_manage)
        preset_manage_layout.setContentsMargins(0, 0, 0, 0)
        preset_manage_layout.addWidget(self.preset_rename)
        preset_manage_layout.addWidget(self.preset_delete)
        preset_manage_layout.addWidget(self.preset_import)
        preset_manage_layout.addWidget(self.preset_export)
        preset_manage_layout.addStretch(1)

        self.preset_status = QLabel()
        self.preset_status.setWordWrap(True)
        preset_group = QGroupBox("Frame Wizard preset library")
        preset_form = QFormLayout(preset_group)
        preset_form.addRow("Preset:", self.preset_combo)
        preset_form.addRow("", preset_primary)
        preset_form.addRow("", preset_manage)
        ...
        self.preset_duplicate.clicked.connect(self._duplicate_selected_preset)
        self.preset_rename.clicked.connect(self._rename_selected_preset)
        self.preset_delete.clicked.connect(self._delete_selected_preset)
        self.preset_import.clicked.connect(self._import_preset_json)
        self.preset_export.clicked.connect(self._export_selected_preset_json)
