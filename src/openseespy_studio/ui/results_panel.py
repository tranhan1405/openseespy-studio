...
class TimeHistoryPlot(QWidget):
    ...
    def paintEvent(self, event) -> None:
        ...
        if (
            ...
            tracker_pen = QPen(QColor("#c62828"), 1)
            tracker_pen.setStyle(Qt.DashLine)
            painter.setPen(tracker_pen)
            painter.drawLine(
                QPointF(marker.x(), top),
                QPointF(marker.x(), bottom),
            )
...
class ResultsPanel(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        root.addWidget(self.motion_page)
    ...
    def show_solution_result(
        ...
        if kind == "Motion":
            ...
            self._select_tab(
                "Mode Shape" if analysis_type == "Modal" else "Deformation"
            )
            self.motion_page.setVisible(
                self._motion_display_frame_count > 0
            )
    ...
    def _build_crack_tab(self) -> None:
        ...
        self.crack_display_button = QPushButton("Display")
        self.crack_display_button.setToolTip(
            "Draw the crack pattern for the currently selected frame."
        )
        self.crack_display_button.clicked.connect(
            self._display_current_crack_frame
        )
        controls.addWidget(self.crack_display_button)

        self.crack_animate_button = QPushButton("▶ Animate")
        self.crack_animate_button.setToolTip(
            "Animate crack evolution and synchronize the crack tables."
        )
        ...
        self.crack_accumulate = QCheckBox("Accumulate")
        self.crack_accumulate.setToolTip(
            "Keep the largest crack state reached up to the current frame."
        )
        ...
        self.crack_detail_tabs = CompactResultTabs()
        self.crack_detail_tabs.setDocumentMode(True)
        layout.addWidget(self.crack_detail_tabs, 1)
        ...
        overview_layout.addWidget(self.crack_summary)

        self.crack_health = QLabel(
            "Data health: no crack instrumentation loaded."
        )
        self.crack_health.setWordWrap(True)
        ...
        self.crack_detail_tabs.addTab(overview, "Overview")

        panels_page = QWidget()
        panels_layout = QVBoxLayout(panels_page)
        panels_layout.setContentsMargins(3, 3, 3, 3)
        ...
        panels_layout.addWidget(self.crack_table, 1)
        self.crack_detail_tabs.addTab(panels_page, "Panels")

        evolution_page = QWidget()
        evolution_layout = QVBoxLayout(evolution_page)
        evolution_layout.setContentsMargins(3, 3, 3, 3)
        self.crack_evolution_table = QTableWidget(0, 5)
        self.crack_evolution_table.setHorizontalHeaderLabels(
            ["Frame", "Valid panels", "Cracked", "Max ε1/εcr", "State"]
        )
        self.crack_evolution_table.horizontalHeader().setSectionResizeMode(
            QHeaderView.ResizeToContents
        )
        self.crack_evolution_table.horizontalHeader().setStretchLastSection(
            True
        )
        self.crack_evolution_table.setEditTriggers(
            QAbstractItemView.NoEditTriggers
        )
        self.crack_evolution_table.setSelectionBehavior(
            QAbstractItemView.SelectRows
        )
        self.crack_evolution_table.setSelectionMode(
            QAbstractItemView.SingleSelection
        )
        self.crack_evolution_table.cellClicked.connect(
            self._crack_evolution_row_clicked
        )
        evolution_layout.addWidget(self.crack_evolution_table, 1)
        self.crack_detail_tabs.addTab(evolution_page, "Evolution")
    ...
    def _crack_principal_strain(values: object) -> float | None:
        ...
        return None if principal is None else float(principal[0])

    def _active_crack_scope(self) -> set[int] | None:
        scope = set(self._active_crack_element_scope)
        return scope or None

    def _write_crack_table(
        self,
        states,
        *,
        accumulated: bool,
        ...
        try:
            ...
            self.crack_table.setRowCount(len(states))
            for row_index, state in enumerate(states):
                values = (
                    str(state.element_tag),
                    str(state.panel),
                    (
                        f"{state.cracking_strain:.6g}"
                        if state.cracking_strain is not None
                        else "-"
                    ),
                    (
                        f"{state.epsilon_1:.6g}"
    ...
    def _update_crack_overview(
        self,
        *,
        states=None,
        frame_label: str = "final state",
        accumulated: bool = False,
    ) -> None:
        scope = self._active_crack_scope()
        current_states = (
            list(states)
            if states is not None
            else mefi_crack_panel_states(
                self._result,
                element_tags=scope,
            )
        )
        summary = mefi_crack_summary(current_states)
        health = mefi_crack_data_health(
            self._result,
            element_tags=scope,
        )
        ...
        self.crack_health.setText(
            ...
            f"valid={int(health['valid'])}/{int(health['panels'])} · "
            f"history={int(health['history_frames'])} frame(s)."
        )

        if int(health["panels"]) == 0:
            self.crack_info.setText(
                "No MEFI crack specification was found in this Job. "
                "Re-run the analysis with the current SARE version."
            )
        elif int(health["thresholds"]) < int(health["panels"]):
            self.crack_info.setText(
                "Some panels have no εcr. Check that each RCLMS concrete "
                "layer references OrthotropicRAConcrete."
            )
    ...
    def _populate_crack_evolution(self) -> None:
        rows = mefi_crack_evolution(
            self._result,
            element_tags=self._active_crack_scope(),
        )
        self.crack_evolution_table.setUpdatesEnabled(False)
        try:
            self.crack_evolution_table.setRowCount(len(rows))
            for row_index, row in enumerate(rows):
                ...
                for column, value in enumerate(values):
                    ...
                    if item is None:
                        item = QTableWidgetItem()
                        self.crack_evolution_table.setItem(
                            row_index,
                            column,
                            item,
                        )
            ...
            self.crack_evolution_table.setUpdatesEnabled(True)
    ...
    def _populate_crack_summary(self) -> None:
        ...
        self._write_crack_table(states, accumulated=False)
    ...
    def _crack_evolution_row_clicked(
        self,
        row: int,
        _column: int,
    ) -> None:
        ...
        try:
            ...
            return
        if self._motion_display_frame_count > 0:
            self._set_motion_index(frame_index)

    def _display_current_crack_frame(self) -> None:
        if self._motion_display_frame_count > 0:
            source_index = self._motion_source_index(
                self._motion_frame_index
            )
    ...
    def _sync_crack_table_frame(self, source_index: int) -> None:
        ...
        states = mefi_crack_panel_states(
            self._result,
            frame_index=max(0, int(source_index)),
    ...
    def _populate_node_table(self) -> None:
        ...
        if hasattr(self, "node_frame_status"):
            self.node_frame_status.setText(
                "Values: final displacement result."
                if displacement
                else "Values: final reaction result."
            )
