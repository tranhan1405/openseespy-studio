...
def build_test_column(
    ...
    effective_integration_type = (
        "Lobatto"
        if spec.element_type == "elasticBeamColumn"
        else str(spec.integration_type)
    )
    if effective_integration_type not in supported_integrations:
        raise ValueError(
            f"Unsupported beam integration type: {effective_integration_type}"
        )
    if effective_integration_type in distributed_integrations:
        ...
        if effective_integration_type in hinge_integrations:
            if (
    ...
    if effective_integration_type in (
        hinge_integrations | {"ConcentratedPlasticity"}
    ):
    ...
    for index in range(count):
        ...
        model.add_element(
            ...
            group="test-column",
            integration_type=effective_integration_type,
            ...
            hinge_i_section_tag=(
                hinge_i_section_tag
                if effective_integration_type
                in hinge_integrations | {"ConcentratedPlasticity"}
                else None
            ),
            hinge_j_section_tag=(
                hinge_j_section_tag
