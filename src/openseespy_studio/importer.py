...
class _Importer:
    ...
    def add_material(self, node: ast.Call, args: list[Any]) -> None:
        ...
        if kind not in MATERIAL_PARAMETER_ORDER:
            ...
            return
        if kind in {"MinMax", "Fatigue", "Parallel", "Series"}:
            self.issue(
                "UNSUPPORTED", node, f"uniaxialMaterial {kind}",
                f"Wrapper material {kind!r} will be added in a later importer pass.",
            )
            return
    ...
    def handle_call(self, command: str, node: ast.Call) -> None:
        try:
            ...
            elif command in {"recorder", "eleLoad", "sp"}:
                self.issue(
                    "UNSUPPORTED", node, command,
                    f"{command} is recognized but not reconstructed in this first importer pass.",
                )
